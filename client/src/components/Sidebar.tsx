...
const Sidebar: React.FC<SidebarProps> = ({ onRouteSubmit }) => {
    ...
    return (
        <div className="absolute top-4 left-1/2 -translate-x-1/2 z-[2000] w-[95%] max-w-5xl">
            {/* Main Dashboard Card */}
            <div className="bg-[#1E2A44] rounded-2xl shadow-2xl p-6 border border-[#3B7BFF] text-[#E7ECF5]">

                {/* Header Section */}
                <div className="flex items-center justify-between mb-6 pb-4 border-b border-[#3B7BFF]/50">
                    <div className="flex items-center gap-4">
                        <div className="bg-[#1E2A44] p-0 rounded-full shadow-md border border-white/5 backdrop-blur-sm overflow-hidden w-12 h-12 flex items-center justify-center">
                ...
                <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-12 gap-3 items-end">
                    ...
                    <div className="md:col-span-3 relative group">
                        ...
                        <div className="relative">
                            <div className="absolute left-4 top-1/2 -translate-y-1/2 text-[#3B7BFF] z-10">
                                <MapPin size={18} />
                            </div>
                            <input
                                type="text"
                                placeholder="Start location..."
                                className="w-full pl-11 pr-4 py-3 bg-[#33402F] border border-[#3B7BFF] rounded-xl focus:outline-none focus:border-[#22D3EE] focus:ring-1 focus:ring-[#22D3EE] text-white placeholder-gray-500 font-medium transition-all shadow-inner"
                                value={start}
                    ...
                    <div className="md:col-span-3 relative group">
                        ...
                        <div className="relative">
                            <div className="absolute left-4 top-1/2 -translate-y-1/2 text-[#22D3EE] z-10">
                                <MapPin size={18} />
                            </div>
                            <input
                                type="text"
                    ...
                    <div className="md:col-span-2 flex gap-2">
                        ...
                        <div className="flex-1">
                            ...
                            <div className="relative">
                                <button
                                    ...
                                    className="w-full bg-[#33402F] border border-[#3B7BFF] hover:bg-[#3d4c38] text-white font-bold py-3 px-2 rounded-xl transition-all shadow-inner h-[50px] flex items-center justify-between px-4 text-xs uppercase tracking-wider"
                                >
                                ...
                                {/* Dropdown Menu */}
                                {showTripTypeMenu && (
                                    <div className="absolute top-full left-0 right-0 mt-2 bg-[#33402F] border border-[#3B7BFF] rounded-xl shadow-2xl p-1 z-50 animate-in fade-in zoom-in-95 duration-200">
                                        <button
                                            type="button"
                                            onClick={() => {
                                                setTripType('one-way');
                                            ...
                                            className={`w-full text-left px-3 py-2 rounded-lg text-xs font-bold uppercase tracking-wider flex items-center justify-between mb-1 transition-colors ${tripType === 'one-way' ? 'bg-[#3B7BFF]/30 text-white' : 'text-gray-400 hover:bg-[#3B7BFF]/10 hover:text-white'}`}
                                        >
                                            <span>One Way</span>
                                            {tripType === 'one-way' && <Check size={14} className="text-[#22D3EE]" />}
                                        </button>
                                        <button
                                            type="button"
                                            onClick={() => {
                                            ...
                                            className={`w-full text-left px-3 py-2 rounded-lg text-xs font-bold uppercase tracking-wider flex items-center justify-between transition-colors ${tripType === 'round-trip' ? 'bg-[#3B7BFF]/30 text-white' : 'text-gray-400 hover:bg-[#3B7BFF]/10 hover:text-white'}`}
                                        >
                                            <span>Round Trip</span>
                                            {tripType === 'round-trip' && <Check size={14} className="text-[#22D3EE]" />}
                                        </button>
                                    </div>
                                )}
                            </div>
                        ...
                        <div className="flex-1">
                            ...
                            <button
                                type="submit"
                                className="w-full bg-[#22D3EE] hover:bg-[#d35400] text-white font-bold py-3 px-4 rounded-xl shadow-lg hover:translate-y-[-2px] active:translate-y-0 transition-all flex items-center justify-center gap-2 h-[50px]"
                            >
                                <Search size={20} />
                            </button>
                        </div>
